...
`/opt/autostream/local-executor/ports/observability.json`; its `bind_address`
is the only local listener authority and its positive `config_revision` is
reported by `/updater/version`. The JSON also carries `schema_version: 2` and
`service_type: observability`. Missing, malformed, writable, or mismatched
credentials fail closed before MariaDB access. There is no bind or revision
...
`127.0.0.1:8082`, while Compose generates `0.0.0.0:8080` by default.
...
Webhook URLs are stored encrypted with `AUTOSTREAM_SECRET_ENCRYPTION_KEY`. Email channels store recipients and a non-secret `control_panel/global_smtp` reference; SMTP credentials remain exclusively in Control Panel. API responses and delivery history expose only non-secret policy, masked targets, fingerprints, status, and timestamps.
